...
- **技术选型调研**：`让 codex 调研一下：数据分析这块我们用 ClickHouse 还是继续用 Postgres？对比查询性能、运维成本、和现有技术栈的契合度，最后给出推荐和理由` —— Codex 自己上网查资料、横向对比、结合你的场景给结论，回来直接能拿去跟团队讨论。
- **跨模块改造**：`让 codex 把用户认证从 JWT 迁移到 OAuth2：改鉴权中间件、补数据库迁移脚本、写单元测试和集成测试，全部跑通后整理一份提交说明` —— Codex 在你当前工作目录里动手改代码、跑测试、交付整套改动，你只负责 review。
- **独立安全审查**：`开个 codex 子 agent 对最近这次提交做安全审查：重点找 SQL 注入、鉴权绕过、敏感信息泄露，给出修复建议，按严重程度排序输出报告` —— 一个和你视角完全不同的"外部专家"，专门负责挑刺，避免自说自话。
